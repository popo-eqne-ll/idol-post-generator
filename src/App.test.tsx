...
describe('Idol Post Generator', () => {
  ...
  test('should generate post text with basic info and selected members', async () => {
    ...
    fireEvent.change(screen.getByLabelText('ライブタイトル'), {
      target: { value: '定期公演' },
    });
    fireEvent.change(screen.getByPlaceholderText('会場名など'), {
      target: { value: '銀河劇場' },
    });
    ...
    // 2. 生成されたテキストエリアを取得
    const generatedTextArea = (await screen.findByRole('textbox', {
      name: '生成された投稿',
  ...
  test('should toggle parentheses for account', async () => {
    ...
    fireEvent.click(screen.getByLabelText('Xアカウントを()で囲う'));
    expect(generatedTextArea.value).toContain(
      '市原 愛弓 (@ichihara_ayumi_)さん',
    );
  });
});
